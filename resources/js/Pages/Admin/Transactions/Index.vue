...
const filterReq = debounce(() => Inertia.get(route('admin.transactions.index'), appended.value, { preserveState: true, replace: true }), 500);
watch(
...
const sortBy = (column) => {
    ...
    Inertia.get(route('admin.transactions.index'), appended.value, { preserveState: true, replace: true });
};
</script>

<template>
    <Head :title="__('Payment attempts')" />
    <!-- Content -->
    <div class="container-xxl flex-grow-1 container-p-y">
        <!-- Invoices List Table -->
        <div class="card">
            <div class="card-header border-bottom">
                <h5 class="card-title mb-3">{{ __('Payment attempts') }}</h5>
                                ...
                                <div class="d-flex justify-content-start align-items-center">
                                    <div class="avatar-wrapper">
                                        <div class="avatar avatar-sm me-3">
                                            <img :src="transaction.member.profile_photo || '/img/user-dark.png'" onerror="this.src = '/img/user-dark.png';" class="rounded-circle" />
                                        </div>
                                    </div>
                                    <div class="d-flex flex-column">
                                        <Link :href="route('admin.members.show', transaction.member.id)" class="text-body text-truncate">
                                            <span class="fw-semibold">{{ transaction.member.fullName }}</span>
                                        </Link>
                                        <small class="text-truncate text-muted">{{ transaction.member.national_id }}</small>
                                    </div>
                                </div>
                            </td>
                            <td>{{ transaction.member.membership_number }}</td>
                            <td>
                                <span
                                    class="badge"
                                    :class="{
                                        'bg-label-success': transaction.member.subscription.num == 1,
                                        'bg-label-secondary': transaction.member.subscription.num == 2,
                                        'bg-label-warning': transaction.member.subscription.num == 3,
                                    }"
                                    text-capitalized=""
                                >
                                    {{ transaction.member.subscription?.type }}
                                </span>
                            </td>
                            <td>{{ transaction.response?.result?.description }}</td>
                            <td>{{ transaction.created_at }}</td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <Link :href="route('admin.transactions.show', transaction.id)" class="text-body" data-bs-placement="top" :aria-label="__('Show')" :title="__('Show')">
