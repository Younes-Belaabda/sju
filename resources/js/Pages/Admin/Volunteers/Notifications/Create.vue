...
const sendNotification = () => {
    ...
    form.post(route('admin.volunteers.notify'));
};
</script>

<template>
    <Head :title="__('Notify volunteers')" />

    <!-- Content -->
    <div class="container-xxl flex-grow-1 container-p-y">
        <!-- User information -->
        <div class="card mb-4">
            <h5 class="card-header">{{ __('Notify volunteers') }}</h5>
            <div class="card-body">
                        ...
                        <div class="mb-3 col-12">
                            ...
                            <div class="d-flex gap-3 flex-wrap">
                                ...
                                <div class="form-check mt-1">
                                    ...
                                    <label class="form-check-label" for="all"> {{ __('All volunteers') }} </label>
                                </div>
                                <div class="form-check mt-1">
                                    <input class="form-check-input" type="radio" name="sendTo" id="active" value="active" v-model="form.to_type" />
                                    <label class="form-check-label" for="active"> {{ __('Active volunteers') }} </label>
                                </div>
                                <div class="form-check mt-1">
                                    <input class="form-check-input" type="radio" name="sendTo" id="inactive" value="inactive" v-model="form.to_type" />
                                    <label class="form-check-label" for="inactive"> {{ __('Inactive volunteers') }} </label>
                                </div>
                            </div>
...
export default {
    mounted() {
        $(document).ready(function () {
            $('.select2').select2({
                ajax: {
                    url: route('admin.volunteers.notify.chuncks'),
                    dataType: 'json',
