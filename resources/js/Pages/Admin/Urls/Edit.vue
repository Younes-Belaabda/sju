...
const form = useForm({
    ...
    link: props.url.link || '',
    target: props.url.target || '_self',
    group: props.url.group || 2,
    _method: 'PUT',
});
</script>

<template>
    <Head :title="__('Edit url')"> </Head>
    <!-- Content -->
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-12 order-0 order-md-1">
                <div class="card mb-4">
                    <h5 class="card-header">{{ __('Edit url') }}</h5>
                    <div class="card-body">
                        <form @submit.prevent="form.post(route('admin.urls.update'))">
                            <!-- Title -->
                            <div class="row">
                                <div class="mb-3 col-12">
                                    <label class="form-label" for="title">{{ __('Title') }}</label>
                                    <div class="input-group">
                                        <input class="form-control" type="text" id="title" :placeholder="__('Title')" v-model="form.title" />
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-12 mb-3">
                                    <label class="form-label" for="group">{{ __('Group') }}</label>
                                    <select class="form-control m-select2-no" id="group" v-model="form.group">
                                        <option value="1">{{ __('Electronic newspaper') }}</option>
                                        <option value="2">{{ __('Paper newspaper') }}</option>
                                    </select>
                                    <p class="fs-6 text-danger" v-if="form.errors.group">{{ form.errors.group }}</p>
                                </div>
                            </div>
                            <!-- Group -->
                            ...
                            <div>
                                ...
                                <Link :href="route('admin.url.index')" as="button" type="reset" class="btn btn-label-secondary me-2">{{ __('Cancel') }}</Link>
                            </div>
                        </form>
                    </div>
                </div>
